...
function findProduct( grid )
{
	let result        = 0;
	let columnsLength = grid[ 0 ].length;
	let rowsLength    = grid.length;

	// rows
	for (let rowIndex = 0; rowIndex < rowsLength; rowIndex += 1)
		for (let columnIndex = 0; columnIndex < columnsLength - 4; columnIndex += 1)
		{
			let multiplication = 
	...
	for (let columnIndex = 0; columnIndex < columnsLength; columnIndex += 1)
		for (let rowIndex = 0; rowIndex < rowsLength - 4; rowIndex += 1)
		{
			let multiplication = 
				grid[ rowIndex ][ columnIndex ] *
...
// 		return columns.map( column => Number( column ));
// 	});
// }

// console.log( toGrid( grid ));